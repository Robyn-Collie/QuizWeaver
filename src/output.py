...
def create_multiple_answers_question(id, title, text, points, options, correct_indices, image_ref=None):
    response_labels = ""
    for idx, opt in enumerate(options):
        response_labels += f"""
          <response_label ident="opt_{idx}">
...
def generate_pdf_preview(questions, filename, quiz_title):
    ...
    for i, q in enumerate(questions):
        ...
        if q_type == "mc":
            ...
            y_position -= 15
            c.drawString(70, y_position, f"[ ] False" if is_true else "[x] False")
            y_position -= 15
...
def create_qti_package(questions, used_images, config):
    ...
    for i, q in enumerate(questions):
        ...
        if q["type"] == "mc":
            question_xml_parts.append(create_mc_question(q_id, q.get("title", "Question"), q.get("text", ""), q.get("points", 5), q.get("options", []), q.get("correct_index", 0), image_ref))
        elif q["type"] == "tf":
            question_xml_parts.append(create_tf_question(q_id, q.get("title", "Question"), q.get("text", ""), q.get("points", 5), q.get("is_true", True), image_ref))
        elif q["type"] == "essay":
            question_xml_parts.append(create_essay_question(q_id, q.get("title", "Question"), q.get("text", ""), q.get("points", 5), image_ref))
    ...
    # Build the manifest
    manifest_content = MANIFEST_TEMPLATE_START.format(manifest_id=manifest_id, assessment_id=assessment_id, assessment_filename=config['qti']['assessment_filename'])
    for _, img_name in used_images:
        manifest_content += f'\n      <file href="{img_name}"/>'
    manifest_content += MANIFEST_TEMPLATE_END
    
    # Write the zip file
    zip_filename = os.path.join(config['paths']['quiz_output_dir'], config['qti']['zip_filename_template'].format(timestamp=timestamp))
