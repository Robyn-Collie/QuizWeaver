...
from src.database import Class, LessonLog, get_engine, get_session, init_db
...
def test_extract_topics_found():
    ...
    text = "Today we studied photosynthesis in depth and also introduced cell division as a preview for next week."
...
def test_assumed_knowledge_max_depth(db_session, sample_class):
    ...
    knowledge = get_assumed_knowledge(db_session, sample_class.id)
    assert knowledge["evolution"]["depth"] == 5, "Depth should be capped at 5 even after 10 lessons"
...
def test_list_lessons_with_topic_filter(db_session, sample_class):
    ...
    photo_lessons = list_lessons(db_session, sample_class.id, filters={"topic": "photosynthesis"})
    assert len(photo_lessons) == 1
    assert "photosynthesis" in photo_lessons[0].content

    gravity_lessons = list_lessons(db_session, sample_class.id, filters={"topic": "gravity"})
...
def test_list_lessons_with_last_days_filter(db_session, sample_class):
    ...
    filtered = list_lessons(db_session, sample_class.id, filters={"last_days": 7})
    assert len(filtered) == 1
    assert filtered[0].content == "Fresh lesson on circuits."

    # Filter to last 30 days - should get both
    all_filtered = list_lessons(db_session, sample_class.id, filters={"last_days": 30})
